...
const carregarAnoAtual = () => {
  const dataAtual = new Date();

  anoAtual.innerText = dataAtual.getFullYear();
};

const exibirMensagem = (textoMensagem, classe) => {
  mensagem.classList.remove("sucesso", "falha");
...
const limparCampoNumero = () => {
  ...
  if (window.screen.width < 1024) numero.blur();
  else numero.focus();
};

const resetarTextoBotaoJogar = () => (botaoJogar.value = "Jogar");
...
const reiniciarJogo = () => {
  ...
  pontuacao.innerText = pontos;
  par.checked = true;
  botaoJogar.value = "Jogar Novamente";
};

const definirVencedor = (valorNumero) => {
...
const validarDados = (event) => {
  ...
  if (
    ...
    !valorNumero ||
    isNaN(valorNumero) ||
    !Number.isInteger(Number(valorNumero)) ||
    valorNumero < 0 ||
    valorNumero > 5
  ) {
    exibirMensagem("Erro: Informe um número inteiro entre 0 e 5!", "falha");
    limparCampoNumero();
  ...
  if (!par.checked && !impar.checked) {
    exibirMensagem("Erro: Selecione uma opção!", "falha");

    return;
  }

  resetarTextoBotaoJogar();
  definirVencedor(valorNumero);
};

document.addEventListener("DOMContentLoaded", () => carregarAnoAtual());

formulario.addEventListener("submit", (event) => validarDados(event));
